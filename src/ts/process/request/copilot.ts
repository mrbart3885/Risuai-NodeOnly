...
const VS_CODE_VERSION = '1.111.0'
const CHAT_VERSION = '0.39.2'
const USER_AGENT = `Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Code/${VS_CODE_VERSION} Chrome/142.0.7444.265 Electron/39.6.0 Safari/537.36`
...
export async function requestCopilot(arg: RequestDataArgumentExtended): Promise<requestDataResponse> {
    ...
    // Sequential rotation: advance before request
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
        ...
        try {
            ...
            if (result.type === 'fail' && keyRotate === 'on-error' && attempt < maxAttempts - 1) {
                advanceKey()
            ...
            // Fix #4: invalidate tid cache on exception too
            if (githubToken) tidCache.delete(githubToken)
            if (keyRotate === 'on-error' && attempt < maxAttempts - 1) {
                advanceKey()
                continue
            }
            return {
